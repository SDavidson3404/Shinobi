...
func _input(event):
	...
	if event.is_action_pressed("ESCAPE"):
		if not is_paused: # If not paused
			...
			# Set Max Health based upon unlocks
			...
			# Set Max Stamina according to unlocks
			...
			# Clear weapon choices
			weapons.clear()
			# For child node in weapon holster
			for child in weapon_holster.get_children():
				# If child is a node
				if child is Node3D:
					# If child is named "sword"
					if child.name == "Sword":
						# Append weapon
						weapons.append(child)
					# Otherwise if child named "spear"
					if child.name == "Spear":
						# If Spear Unlocked
						if SkillManager.check_unlocked("spear_unlock"):
							# Append Weapon
							weapons.append(child)
					# Otherwise if child named "dagger"
	...
	if Input.is_action_pressed("dodge"):
		if SkillManager.check_unlocked("dodge"):
			if not is_paused:
				try_dodge()
	# If you press space and are wall running:
	...
	if Input.is_action_just_pressed("Space"):
		...
		if can_jump:
			...
			Current_jumps += 1
			# If current jumps is 1 and double jump unlocked
			if Current_jumps == 1 and SkillManager.check_unlocked("double_jump"):
				# Can jump equals true
				can_jump = true
...
func _physics_process(delta: float) -> void:
	...
	if can_wall_run and not is_on_floor():
...
func handle_movement(delta: float) -> void:
	...
	if is_dodging or is_wall_climbing:
		return
...
func try_dodge() -> void:
	...
	if is_dodging or dodge_cooldown_timer > 0:
		return
...
func _on_close_menu_button_down() -> void: # On close menu pressed
	...
	# Clear weapons
	weapons.clear()
	# For child of weapon holster
	for child in weapon_holster.get_children():
		# If child is a node
		if child is Node3D:
			# If child named "sword"
			if child.name == "Sword":
				# Append weapon
				weapons.append(child)
			# If child name is "spear"
			if child.name == "Spear":
				# If unlocked spear
				if SkillManager.check_unlocked("spear_unlock"):
					# Append Weapon
					weapons.append(child)
			# If child named "dagger"
			if child.name == "Dagger":
				# If dagger unlocked
				if SkillManager.check_unlocked("dagger_unlock"):
					# Append weapon
			...
			# Set child to not visible
			child.visible = false
		# Set current weapon to first in list
		if weapons.size() > 0:
			current_weapon_index = 0
			current_weapon = weapons[current_weapon_index]
			current_weapon.visible = true
