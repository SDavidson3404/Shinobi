...
# === STATS ===
@export var light_damage: int = 10 # Original Light attack damage
@export var heavy_damage: int = 15 # Original Heavy attack damage
...
func _ready() -> void:
	# Connect to collectible collected
...
func _on_hitbox_body_entered(body: Node3D) -> void:
	...
	# While enemy is not in group "enemies":
	while enemy_node and not enemy_node.is_in_group("enemies"):
		# Get the parent node of the enemy
...
# ON COLLECTED
# ========================
# Function runs when collecting collectible
func _on_collected():
	# Up attack damage
	self.light_damage += 1
	self.heavy_damage += 1
