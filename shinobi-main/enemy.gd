...
func _ready():
	...
	if players.size() > 0:
		target = players[0]
	# If sword has method enable damage
...
func handle_ai_movement(delta):
	...
	# If distance is lower than detection range
	if distance <= detection_range:
		# Is alerted is true
		is_alerted = true
	# Else, if distance is higher than forget range:
...
func try_attack():
	...
	if anim_player.has_animation("attack"):
		...
		# While animation isnt finishedL
		while finished_anim != "attack":
			# Wait for finished animation
...
func apply_knockback(force: Vector3) -> void: knockback_velocity = force # Set knockback velocity
